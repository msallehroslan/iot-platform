...
function Sidebar({ page, setPage, user, onLogout, alarmCount }) {
  ...
  return (
    <aside className={`${col?"w-14":"w-56"} flex-shrink-0 flex flex-col h-screen transition-all duration-200 border-r border-slate-200`}>
      <div className="flex items-center gap-3 px-4 py-5 border-b border-slate-100 overflow-hidden">
        ...
        {!col && <span className="font-bold text-slate-800 text-sm tracking-wide truncate">TriAxis IoT</span>}
      </div>
      <nav className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        {!col && <p className="px-3 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-widest text-slate-400">Menu</p>}
        {NAV.map(({id,label,icon}) => (
          <button key={id} onClick={() => setPage(id)} title={col?label:undefined}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${page===id?"bg-blue-50 text-blue-600":"text-slate-500 hover:bg-slate-100 hover:text-slate-800"}`}>
