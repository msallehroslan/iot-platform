...
from app.models.models import (
    Device, LatestTelemetry, TelemetryData, TelemetryKey,
...
# ── Per-key intelligence (Gap 1 — standard KeyIntelligence schema) ────────────

def get_key_intelligence(
    db,
    device_id: str,
    key: str,
    device=None,
    ...
    """
    Standard per-key intelligence contract consumed by both TAAT tools and widgets.

    This is the atomic unit of the intelligence layer — one key, fully enriched.
    Both the unified summary and widget data API return arrays of these.

    Returns KeyIntelligence dict:
        {
            "key":                str,
            "value":              float | str | bool | None,
            "unit":               str | None,          # from TelemetryKey metadata
            "baseline_min":       float | None,        # suggested_lower (±3σ)
            "baseline_max":       float | None,        # suggested_upper (±3σ)
            "baseline_mean":      float | None,
            "baseline_status":    "active" | "learning",
            ...
            "ts":                 str | None,          # ISO timestamp of latest value
        }
    """
    # ── Fetch all layers (all cached individually) ───────────────────────────
    ...
    # ── Unit from TelemetryKey metadata ──────────────────────────────────────
    unit = None
    try:
        tk = db.query(TelemetryKey).filter(
    ...
    b               = baseline.get("keys", {}).get(key) if baseline_status == "active" else None
    baseline_min    = b.get("lower")  if b else None
    baseline_max    = b.get("upper")  if b else None
    baseline_mean   = b.get("mean")   if b else None

    # ── Trend ─────────────────────────────────────────────────────────────────
    trend          = trend_data.get("trend", "UNKNOWN")
    trend_change   = trend_data.get("change_pct", 0.0)

    # ── Anomaly ───────────────────────────────────────────────────────────────
    is_anomaly   = False
    anomaly_z    = None
    most_anom    = anomaly.get("most_anomalous_key")
    anom_count   = anomaly.get("anomaly_count", 0)
    recent       = anomaly.get("recent_anomalies", [])
    key_anomalies = [a for a in recent if a["key"] == key]
    if key_anomalies:
        is_anomaly = True
        anomaly_z  = key_anomalies[0].get("z_score")
    elif most_anom == key and anom_count > 0:
        is_anomaly = True
    ...
    if num is not None:
        # Check against baseline bounds
        above_upper = baseline_max is not None and num > baseline_max
        below_lower = baseline_min is not None and num < baseline_min
        above_mean  = baseline_mean is not None and num > baseline_mean
        below_mean  = baseline_mean is not None and num < baseline_mean

        if above_upper or below_lower:
            status = "CRITICAL" if is_anomaly else "WARNING"
            risk   = "HIGH"     if is_anomaly else "MEDIUM"
            direction = "above" if above_upper else "below"
            bound     = baseline_max if above_upper else baseline_min
            reason = (
                f"{key} {num:.1f} {unit or ''} is {direction} normal range "
                f"({baseline_min:.1f}–{baseline_max:.1f} {unit or ''})"
                if (baseline_min is not None and baseline_max is not None)
                else f"{key} is outside baseline"
            ).strip()

            if trend in ("RISING", "SPIKE") and above_upper:
                risk   = "CRITICAL"
                status = "CRITICAL"
                reason += " and still rising"
                recommended_action = f"Investigate {key} immediately — rising beyond normal range"
            elif trend in ("FALLING", "DROP") and below_lower:
                risk   = "CRITICAL"
                status = "CRITICAL"
                reason += " and still falling"
                recommended_action = f"Check {key} — dropping below normal range"
            else:
                recommended_action = f"Monitor {key} — outside normal operating range"

        elif is_anomaly:
            status = "WARNING"
            risk   = "MEDIUM"
            reason = f"{key} detected as anomalous (z={anomaly_z:.2f})" if anomaly_z else f"{key} anomaly detected"
            recommended_action = f"Review recent {key} readings for sensor issues"

        elif trend in ("SPIKE", "DROP", "VOLATILE"):
            status = "WARNING"
            risk   = "MEDIUM"
            reason = f"{key} shows {trend.lower()} pattern"
            recommended_action = f"Watch {key} — unstable readings"

        elif above_mean and trend == "RISING":
            status = "WARNING"
            risk   = "LOW"
            reason = f"{key} above mean ({baseline_mean:.1f}) and rising"
        else:
            status = "NORMAL"
            risk   = "LOW"
            reason = (
                f"{key} within normal range ({baseline_min:.1f}–{baseline_max:.1f} {unit or ''})"
                if (baseline_min is not None and baseline_max is not None)
                else f"{key} operating normally"
            ).strip()
    elif value is not None:
        # Non-numeric (boolean / string) — just report current state
        status = "NORMAL"
        risk   = "LOW"
        reason = f"{key} = {value}"
    ...
    return {
        ...
        "value":              value,
        "unit":               unit,
        "baseline_min":       round(baseline_min,  4) if baseline_min  is not None else None,
        "baseline_max":       round(baseline_max,  4) if baseline_max  is not None else None,
        "baseline_mean":      round(baseline_mean, 4) if baseline_mean is not None else None,
        "baseline_status":    baseline_status,
        "trend":              trend,
        "trend_change_pct":   round(trend_change, 2),
        "anomaly":            is_anomaly,
        "anomaly_z_score":    round(anomaly_z, 4) if anomaly_z is not None else None,
        "risk":               risk,
        "status":             status,
        "reason":             reason,
