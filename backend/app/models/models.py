...
from sqlalchemy import (
    ...
    Text, Enum, Integer, JSON, UniqueConstraint
)
...
class LatestTelemetry(Base):
    ...
    # Enforce one row per (device, key) at the DB level so concurrent HTTP + MQTT
    # ingest can never create duplicate rows via the read-then-write upsert.
    __table_args__ = (
...
# ── Dashboard & Widget Models (Gap 1 + Gap 2) ─────────────────────────────────
...
class Widget(Base):
    ...
    widget_type  = Column(String(50), nullable=False)   # value_card|line_chart|gauge|status_light|bar_chart|alarm_list|timeseries_table|pie_chart|markdown|entity_table|html_card
    title        = Column(String(255), nullable=False, default="Widget")
    config       = Column(JSON, nullable=False, default=dict)   # key, unit, color, min, max, decimals, threshold_high, keys[], content
    position     = Column(JSON, nullable=False, default=lambda: {"x": 0, "y": 0, "w": 2, "h": 3})  # react-grid-layout coords
    created_at   = Column(DateTime(timezone=True), server_default=func.now())
    updated_at   = Column(DateTime(timezone=True), onupdate=func.now())
...
# ── User-Scoped Dashboard & Widget Models (Phase 2: Multi-Dashboard) ──────────
# These are SEPARATE from the device-scoped Dashboard/Widget above.
# They belong to a user (via user_id string), not a device.
# Supports the ThingsBoard-style "manage all dashboards" sidebar panel.

class UserDashboard(Base):
    __tablename__ = "user_dashboards"

    id          = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    user_id     = Column(String(255), nullable=False, index=True)   # JWT sub claim (user UUID as string)
    name        = Column(String(255), nullable=False)
    description = Column(Text, nullable=True)
    ...
    widgets = relationship(
        "UserWidget", back_populates="dashboard",
        cascade="all, delete-orphan",
        order_by="UserWidget.created_at"
    )
