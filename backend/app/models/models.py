...
# ── TAAT v2: Agent Memory ─────────────────────────────────────────────────────

class AgentMemory(Base):
    """
    Persistent TAAT memory. Stores:
        device_context  — "Pump-01 usually runs at night"
        user_pref       — "User prefers daily report at 8 AM"
        incident        — "ESP32-001 temperature spike 2025-01-15"
        device_alias    — "ESP32-e823 controls test LED"
    Written by TAAT on notable events. Read at the start of every chat turn.
    """
    ...
    user_id     = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="SET NULL"),
                         nullable=True)
    memory_type = Column(String(50), nullable=False)   # device_context|user_pref|incident|device_alias
